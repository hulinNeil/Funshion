<template>
	<div class="player" ref="player" id="player">
		<div class="player-cont">
			<div style="height: 100%;width: 100%;background: rgba(0,0,0,0.7);"></div>
			<video :src="videoUrl" ref="video"></video>
			<div class="play" @click="showControl">
				<img v-if="!isPlay" src="./../img/play1.png" @click.stop="play" />
			</div>
			<div class="v-controls" ref="aa">
				<a class="v-controls-play" :class="isPlay ? 'pause' : ''" @click="play"></a>
				<span class="v-controls-time">{{currentTime}}</span>
				<div class="v-controls-progress">
					<input type="range" ref="progressSlider" class="progress-slider" value="0" step="0.01" :style="{backgroundSize:currentTimeLength}" />
				</div>
				<span class="v-controls-time">{{duration}}</span>
				<div class="v-controls-hd">高清</div>
				<a ref="fullscreen" class="v-controls-fullscreen" :class="isFullscreen  ? 'fullscreen' : ''" @click="fullscreen"></a>
			</div>
		</div>
	</div>
</template>

<script>
	export default {
		name: "videoPlayer",
		data() {
			return {
				data: {},
				isPlay: false,
				isFullscreen: false,
				showControls: true,
				duration: "00:00",
				currentTime: "00:00",
				currentTimeLength: "0% 100%",
				progressSliderLeft: 0
			}
		},
		props: {
			videoUrl: {
				type: String,
				default: ""
			}
		},
		created() {
			this.$nextTick(function() {
				let video = document.querySelector("video"),
					player = this.$refs.player,
					progressSlider = this.$refs.progressSlider,
					fullscreen = this.$refs.fullscreen,
					durationTime = 0,
					onInputChange = false;
				video.addEventListener('loadedmetadata', (e) => {
					this.duration = this.stom(e.target.duration);
					durationTime = e.target.duration;
				})
				video.addEventListener('timeupdate', (e) => {
					if(!onInputChange && e.target.duration) { //当change的时候这里播放，不需要记录；
						this.currentTime = this.stom(e.target.currentTime);
						this.currentTimeLength = (e.target.currentTime / e.target.duration) * 100 + '%' + ' 100%';
						progressSlider.value = e.target.currentTime / e.target.duration * 100;
					}
				})
				video.addEventListener('ended', (e) => {
					this.initPlayer();
				})
				progressSlider.addEventListener("change", (e) => { //改变video的播放时间
					onInputChange = false;
					let value = e.target.value;
					video.currentTime = value / 100 * durationTime;
				});
				progressSlider.addEventListener("input", (e) => { //改变现实的时间
					onInputChange = true;
					let value = e.target.value;
					this.currentTime = this.stom(value / 100 * durationTime);
					this.currentTimeLength = value + '%' + ' 100%';
				});
				fullscreen.addEventListener('click', () => { //进入全屏
					if(player.requestFullscreen) {
						player.requestFullscreen();
					} else if(player.mozRequestFullScreen) {
						player.mozRequestFullScreen();
					} else if(player.msRequestFullscreen) {
						player.msRequestFullscreen();
					} else if(player.webkitRequestFullscreen) {
						player.webkitRequestFullScreen();
					}
				})
			});
			this.lisenTouch();
		},
		watch: {
			"videoUrl": function(e) {
				this.initPlayer();
			},
			"isPlay": function() {
//         let video = document.querySelector("#player video");
// 				if(!this.isPlay) {
//           video.pause();
// 					// this.$refs.video.pause();
// 				} else {
// 					video.play();
//           // this.$refs.video.play();
// 				}
			},
			"showControls": function() {
				if(this.showControls) {
					this.$refs.aa.setAttribute("style", "bottom:0px;transition: all 300ms;");
				} else {
					this.$refs.aa.setAttribute("style", "bottom:-2.4rem;transition: all 300ms;");
				}
			},
			"isFullscreen": function() {
				if(this.isFullscreen) {
					console.log("此时应该全拼")
				} else {
					if(document.exitFullscreen) {
						document.exitFullscreen();
					} else if(document.msExitFullscreen) {
						document.msExitFullscreen();
					} else if(document.mozCancelFullScreen) {
						document.mozCancelFullScreen();
					} else if(document.webkitCancelFullScreen) {
						document.webkitCancelFullScreen();
					}
				}
			},
		},
		methods: {
			play() {
        if(this.$refs.video.networkState === 3){
          this.$refs.video.src = this.$refs.video.src;
        }
				this.isPlay = !this.isPlay;
        if(this.isPlay){
          document.querySelector("#player video").play();
        }else{
          document.querySelector("#player video").pause();
        }
        
			},
			fullscreen() {
				this.isFullscreen = !this.isFullscreen;
			},
			stom(t) {
				var m = Math.floor(t / 60);
				m < 10 && (m = '0' + m);
				return m + ":" + (t % 60 / 100).toFixed(2).slice(-2);
			},
			showControl() {
				if(this.isPlay) {
					this.showControls = !this.showControls
				}
			},
			initPlayer() {
				console.log("初始化播放器！！");
				this.isPlay = false, this.isFullscreen = false, this.showControls = true,
					this.currentTime = "00:00", this.currentTimeLength = "0% 100%",
					this.progressSliderLeft = 0;
				let progressSlider = this.$refs.progressSlider;
				progressSlider.value = 0;
			},
			lisenTouch(){//监听滑动事件，改变进度，音量，光感
        let sound = "data:image/png;base64,";
        let sun = "data:image/png;base64,"
        let speed = "data:image/png;base64,";
        let backward = "data:image/png;base64,";
				
        let startX=null,startY=null,endX=null,endY=null,moveLength=null,direction = 'X',isFirstMove = true;
				let video = null,videoLength=null,videoCurrentTime=null,videoCurrentTimeTom=null,videoCurrentTimeTo=null;
        let base64Img = null/*图片*/,append_text = null/*文字*/,moveX = null,moveXlast = null,moveY = null,moveYlast = null,fullElement = null/*全屏元素,touchend不同重置*/,getSystemV = null/*声音*/,getSystemS = null/*亮度*/;
        let startTime = null/*触摸屏幕的时间ms*/,moveTime = null/*此时正在触摸时的时间*/,moveLastTime = null/*上一次move的时间*/;
        document.addEventListener('webkitfullscreenchange', function() {//监听
          fullElement = document.webkitFullscreenElement; //获取全屏元素
          if(fullElement) {
            document.addEventListener('touchstart',startF,true);
            document.addEventListener('touchmove',moveF,true);
            document.addEventListener('touchend',endF,true);
          } else {
            document.removeEventListener('touchstart',startF,true);
            document.removeEventListener('touchmove',moveF,true);
            document.removeEventListener('touchend',endF,true);
          }
        });
        
        function startF(e){
          startTime = new Date().getTime();
          startX = e.touches[0].pageX,startY = e.touches[0].pageY;
          if(window.innerHeight-startY<100){
            return;
          }
					if(fullElement.tagName === 'VIDEO'){
						video = fullElement;
					}else{
            let target = fullElement;
            for(; target && target !== document; target = target.parentNode) {
              // console.log(fullElement,target);
                if(target.querySelector("video")) {
                    video = target.querySelector("video");
                    break;
                }
            }
					}
          videoLength = video.duration,videoCurrentTime = video.currentTime;
          // console.log("初次触摸的时间：",setTimeLength(videoCurrentTime));
        }
        function moveF(e){
          let mm = null;//单位滑动距离y
          if(!moveLastTime){//得到时间，根据时间来设置滑动的距离
            moveLastTime = startTime
          }else{
            moveLastTime = moveTime
          }
          moveTime = new Date().getTime();
					endX = e.touches[0].pageX,endY = e.touches[0].pageY;
          if(window.innerHeight-endY<100 ){//如果得不到video或触摸距离在底部100px，则不滑动
            return;
          }
          if(!video){
            // console.log("得不到video~~~");
            if(fullElement.tagName === 'VIDEO'){
            	video = fullElement;
            }else{
            	let target = fullElement;
            	for(; target && target !== document; target = target.parentNode) {
            		// console.log(fullElement,target);
            			if(target.querySelector("video")) {
            					video = target.querySelector("video");
            					break;
            			}
            	}
            }
            videoLength = video.duration,videoCurrentTime = video.currentTime;
          }
          if(!video){
            return;
          }
					if(isFirstMove){//是move的第一次，下面判断方向；
						if(Math.abs(endX-startX) < Math.abs(endY-startY)){
							direction = 'Y';
						}
            if(direction === 'X'){//处理第一次滑动的图片
              if(endX-startX>=0){
                base64Img = speed;
              }else{
                base64Img = backward;
              }
            }else{
              if(window.innerWidth/2>endY){//是在左边滑动
                base64Img = sun;
              }else{
                base64Img = sound;
              }
            }
						isFirstMove = false;
					}
					if(direction === 'X'){
						moveLength = endX-startX;
					}else{//如果是Y的话，需要实时的改变滑动的距离，x不需要，只要最后改变就行了。
            moveLength = endY-startY;
            if(!moveYlast){//得到时间，根据时间来设置滑动的距离
            	moveYlast = startY
            }
            mm = endY-moveYlast;
            moveYlast = endY
					}
          
          if(videoLength < 300 && direction === 'X'){
            moveLength = moveLength/2
          }
          
          videoCurrentTimeTo = videoCurrentTime+moveLength;
          
          if(direction === 'X'){//处理图片和文字
            if(videoCurrentTimeTo <= 0){//当videoCurrentTimeTo＜0时冻结数据，只要朝正方向滑动就要加，朝反方向不管。大于最大时间也一样。
              videoCurrentTimeTo = 0;
            }else if(videoCurrentTimeTo >= videoLength){
              videoCurrentTimeTo = videoLength;
            }
            append_text = setTimeLength(videoCurrentTimeTo);
            if(endX-startX >=0){
            	base64Img = speed;
            }else{
            	base64Img = backward;
            }
          }else{
            if(!window.plus){
              return;
            }
            if(window.innerWidth/2>endX){//是在左边滑动
              base64Img = sun;
              append_text = setMark(moveLength,'sun');
            }else{
              base64Img = sound;
              append_text = setMark(moveLength,'sound');
            }
          }
          // console.log("video当前时间：",videoCurrentTime,"滑动方向",direction,"当前滑动长度：",moveLength,"单位滑动距离",mm,",单位速度是：",moveLength/(startTime - moveLastTime));
          // console.log("toast显示的文字：",append_text);
          let WIN_W = window.innerWidth,_left = (WIN_W-120)/2;
          
          let html = `
              <div style="box-sizing:content-box;width:100px;height:30px;padding:0 10px;line-height:30px;background:rgba(0,0,0,0.5);position:fixed;z-index:99999999999;top:50%;left:${_left}px;border-radius:20px;text-align:center;">
                  <img id="append_touch_img" style="width:30px;height:30px;float:left;" src="${base64Img}"/>
                  <span id="append_touch_text" style="color:#ffffff;">${append_text}</span>
              </div>
          `
          if(!document.querySelector("#append_touch_div")){
            let apend = document.createElement("div");
            apend.setAttribute("id","append_touch_div")
            apend.innerHTML = html;
            video.parentNode.appendChild(apend);
          }else{
            let append_touch_text = document.querySelector("#append_touch_text");
            append_touch_text.innerText = append_text;
            if(direction == 'X'){//判断滑动的方向是否是X,是的话就要考虑换img的src
              document.querySelector("#append_touch_img").src = base64Img;
            }
          }
          e.preventDefault();
          e.stopPropagation();
        }
        function endF(e){
          let apend = document.querySelector("#append_touch_div");
          if(apend){
            if(direction === 'X'){
              video.currentTime = videoCurrentTimeTo;
            }
            startX=null,startY=null,endX=null,endY=null,moveLength=null,direction = 'X',isFirstMove = true;
            video = null,videoLength=null,videoCurrentTime=null,videoCurrentTimeTom=null,videoCurrentTimeTo=null;
            base64Img = null/*图片*/,append_text = null/*文字*/,moveX = null,moveXlast = null,moveY = null,moveYlast = null,getSystemV = null/*声音*/,getSystemS = null/*亮度*/;
            apend.remove();
          }
        }
        function setTimeLength(t){
            var m = Math.floor(t / 60);
            m < 10 && (m = '0' + m);
            return m + ":" + (t % 60 / 100).toFixed(2).slice(-2);
        }
        function setMark(t,type){//得到了
          let mark = 0;
          if(window.plus){
            if(type === 'sun' && !getSystemS){
              getSystemS = plus.screen.getBrightness();/*设置亮度的api在流基座里面崩溃*/
            }else if(type === 'sound' && !getSystemV){
              getSystemV = plus.device.getVolume();/*取set的值然后保存显示*/
            }
            if(type === 'sun'){
            	mark = getSystemS
            }else{
            	mark = getSystemV
            }
            mark = (mark - t/300).toFixed(2);/*3px转化为1个点0.01*/
            if(mark>1){
              mark = 1;
            }else if(mark < 0){
              mark = 0;
            }
            if(type === 'sun'){
              plus.screen.setBrightness(mark);
            }else{
              plus.device.setVolume(mark);
            }
          }
          return Math.ceil(mark*100) + '%';
        }
			}
		}
	}
</script>

<style>
   
	.player {
		background: #000;
		height: 11.25rem;
		width: 100%;
		position: relative;
        z-index:2;
	}
	
	.player-cont {
		position: fixed;
		width: 20rem;
		height: 11.25rem;
		overflow: hidden;
	}
	
	video {
		width: 20rem;
		height: 11.25rem;
		background: #000000;
		position: fixed;
		top: 0;
		left: 0;
		z-index: 2;
	}
	
	.play {
		height: 11.25rem;
		width: 20rem;
		position: fixed;
		z-index: 3;
		top: 0;
		left: 0;
	}
	
	.play img {
		width: 4rem;
		height: 4rem;
		margin-top: 3.625rem;
	}
	
	.v-controls {
		position: absolute;
		z-index: 10;
		left: 0;
		bottom: 0;
		display: flex;
		width: 100%;
		height: 2.4rem;
		line-height: 2.4rem;
		/*visibility: hidden;*/
		opacity: 1;
		color: #fff;
		background: rgba(0, 0, 0, .6);
	}
	
	.v-controls-play {
		width: 1.2rem;
		background-image: url(./../img/playBtn.png);
		background-repeat: repeat;
		background-size: 6.08rem;
		background-position: -4.4rem -6.29rem;
	}
	
	.v-controls-play.pause {
		background-position: -4.4rem -11.02rem;
	}
	
	.v-controls-time {
		width: 2rem;
		font-size: 0.6rem;
	}
	
	.v-controls-progress {
		flex: 1;
		display: flex;
		flex-direction: column;
		justify-content: center;
	}
	
	.v-controls-progress .progress-slider {
		outline: none;
		width: 100%;
		height: 2px;
		-webkit-appearance: none;
		/*清除系统默认样式*/
		background: -webkit-linear-gradient(#61bd12, #61bd12) no-repeat, #ddd;
		/*设置左边颜色为#61bd12，右边颜色为#ddd*/
		background-size: 0% 100%;
		/*设置左右宽度比例*/
	}
	/*拖动块的样式*/
	
	input[type=range]::-webkit-slider-thumb {
		-webkit-appearance: none;
		/*清除系统默认样式*/
		height: 16px;
		/*拖动块高度*/
		width: 16px;
		/*拖动块宽度*/
		background: #fff;
		/*拖动块背景*/
		border-radius: 50%;
		/*外观设置为圆形*/
	}
	
	.v-controls-hd {
		width: 1.8rem;
		font-size: 0.7rem;
		text-align: end;
	}
	
	.v-controls-fullscreen {
		width: 1.6rem;
		background-image: url(./../img/playBtn.png);
		background-repeat: repeat;
		background-size: 6.08rem;
		background-position: -4.4rem -20.51rem;
	}
	
	.v-controls-fullscreen.fullscreen {
		background-position: -4.4rem -25.1rem;
	}
</style>